...
export default function DesktopApp(){
  ...
  const AiChat=()=><div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 100px)"}}>
    ...
    <div ref={chatRef} style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:10,paddingBottom:10}}>
      {msgs.map((m,i)=>{
        if(m.r==="ai")return <div key={i} style={{maxWidth:"70%",padding:"14px 18px",borderRadius:"4px 16px 16px 16px",background:C.card,border:`1px solid ${C.brd}`,boxShadow:"0 1px 3px rgba(0,0,0,0.03)"}}><div style={{display:"flex",alignItems:"center",gap:4,marginBottom:6}}><I.Spark z={11} style={{color:C.pri}}/><span style={{fontSize:10,fontWeight:700,color:C.pri}}>HealthGen IA</span></div><div style={{fontSize:13,color:C.tx2,lineHeight:1.6,whiteSpace:"pre-wrap"}} dangerouslySetInnerHTML={{__html:m.t.replace(/\*\*(.*?)\*\*/g,'<b style="color:#0F1117">$1</b>')}}></div></div>;
  ...
  return (<div style={{minHeight:"100vh",display:"flex",background:`linear-gradient(180deg,${C.bg},${C.bg2})`,fontFamily:"'Plus Jakarta Sans',-apple-system,sans-serif"}}>
    ...
    <div style={{width:240,minHeight:"100vh",background:C.card,borderRight:`1px solid ${C.brd}`,display:"flex",flexDirection:"column",position:"fixed",top:0,left:0,zIndex:30}}>
      ...
      <div style={{padding:"20px 20px 16px",display:"flex",alignItems:"center",gap:8}}>
        ...
        <span style={{fontSize:16,fontWeight:900,color:C.tx}}>Health<span style={{color:C.pri}}>Gen</span></span>
      </div>
