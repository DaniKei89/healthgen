...
export default function DesktopApp(){
  ...
  const tabs=[{id:"d",l:"Home",ic:I.Heart},{id:"f",l:"Familia",ic:I.DNA},{id:"e",l:"Datos",ic:I.Act},{id:"a",l:t("nav.ai"),ic:I.Chat},{id:"t",l:"Tips",ic:I.Leaf},{id:"w",l:t("nav.wearables"),ic:I.Watch},{id:"o",l:"Docs",ic:I.File},{id:"p",l:"Perfil",ic:I.User}];
  ...
  const AiChat=()=><div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 100px)"}}>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}><div style={{width:40,height:40,borderRadius:12,background:`linear-gradient(135deg,${C.pri},${C.pur})`,display:"flex",alignItems:"center",justifyContent:"center",animation:"pulse 2s ease-in-out infinite"}}><I.Spark z={18} sk="white"/></div><div><h2 style={{fontSize:20,fontWeight:800,color:C.tx}}>{t("chat.title")}</h2><p style={{fontSize:11,color:C.tx3}}>{t("chat.subtitle")}</p></div></div>
    <div ref={chatRef} style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:10,paddingBottom:10}}>
      {msgs.map((m,i)=>{
        if(m.r==="ai")return <div key={i} style={{maxWidth:"70%",padding:"14px 18px",borderRadius:"4px 16px 16px 16px",background:C.card,border:`1px solid ${C.brd}`,boxShadow:"0 1px 3px rgba(0,0,0,0.03)"}}><div style={{display:"flex",alignItems:"center",gap:4,marginBottom:6}}><I.Spark z={11} style={{color:C.pri}}/><span style={{fontSize:10,fontWeight:700,color:C.pri}}>{t("chat.aiLabel")}</span></div><div style={{fontSize:13,color:C.tx2,lineHeight:1.6,whiteSpace:"pre-wrap"}} dangerouslySetInnerHTML={{__html:m.t.replace(/\*\*(.*?)\*\*/g,'<b style="color:#0F1117">$1</b>')}}></div></div>;
    ...
    <div style={{display:"flex",gap:8,padding:"8px 0"}}><input value={inp} onChange={e=>setInp(e.target.value)} onKeyDown={e=>e.key==="Enter"&&sendMsg(inp)} placeholder={t("chat.placeholder")} style={{flex:1,padding:"12px 18px",borderRadius:12,border:`1.5px solid ${C.brd}`,background:C.card,fontSize:13,color:C.tx,outline:"none"}}/><button onClick={()=>sendMsg(inp)} style={{width:46,height:46,borderRadius:12,border:"none",cursor:"pointer",background:`linear-gradient(135deg,${C.pri},${C.pur})`,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:`0 2px 8px ${C.pri}30`}}><I.Send z={17} sk="white"/></button></div>
  </div>;
  ...
  const Prof=()=><div style={{display:"flex",flexDirection:"column",gap:16}}>
    ...
    {/* Language Toggle */}
    <Cd style={{padding:"14px 18px"}}><div style={{display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:18}}>🌐</span><div style={{flex:1}}><p style={{fontSize:13,fontWeight:700,color:C.tx}}>Idioma / Language</p></div><div style={{display:"flex",gap:5}}>{[{code:"es",label:"ES"},{code:"en",label:"EN"}].map(lng=><button key={lng.code} onClick={()=>i18n.changeLanguage(lng.code)} style={{padding:"5px 14px",borderRadius:8,fontSize:12,fontWeight:i18n.language?.startsWith(lng.code)?700:500,cursor:"pointer",background:i18n.language?.startsWith(lng.code)?C.pri:"transparent",color:i18n.language?.startsWith(lng.code)?"white":C.tx2,border:`1px solid ${i18n.language?.startsWith(lng.code)?C.pri:C.brd}`}}>{lng.label}</button>)}</div></div></Cd>
